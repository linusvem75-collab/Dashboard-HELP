...
REQUIRED_COLUMNS = [
    "Order Date", "Ship Date", "Sales", "Profit", "UnitPrice", "Discount",
    "Product Category", "Product Sub-Category", "Customer Segment",
    "Region & District", "Order ID",
]


@st.cache_data(show_spinner="Loading and cleaning data...")
def load_and_clean(file_or_path) -> pd.DataFrame:
    # --- detect a Git LFS pointer file masquerading as the CSV ---
    # If a CSV is tracked by Git LFS but the deploy environment doesn't fetch
    # LFS objects (Streamlit Cloud does NOT by default), the "file" that gets
    # checked out is a ~130-byte text pointer, not real data. Reading that as
    # a CSV parses "successfully" but produces nonsense columns, which is a
    # very plausible explanation for a KeyError on a column that should
    # obviously be there.
    try:
        if hasattr(file_or_path, "read"):
            head = file_or_path.read(200)
            file_or_path.seek(0)
            head_text = head.decode("utf-8", errors="ignore") if isinstance(head, bytes) else head
        else:
            with open(file_or_path, "rb") as fh:
                head_text = fh.read(200).decode("utf-8", errors="ignore")
        if head_text.startswith("version https://git-lfs.github.com/spec"):
            st.error(
                "**This CSV is a Git LFS pointer file, not the actual data.** "
                "The file is tracked by Git LFS in your repo, but Streamlit "
                "Community Cloud does not fetch LFS objects during deploy — "
                "so it checks out a tiny placeholder instead of the real CSV.\n\n"
                "**Fix:** remove the CSV from LFS tracking (check `.gitattributes` "
                "for a `*.csv filter=lfs` rule and remove it), then re-commit the "
                "actual file with `git lfs migrate export` or by re-adding it "
                "after untracking, and push again."
            )
            st.stop()
    except Exception:
        pass  # best-effort sniff only; fall through to normal loading/errors below
    ...
    # --- validate required columns are present, with a diagnostic error instead of a bare KeyError ---
    missing = [c for c in REQUIRED_COLUMNS if c not in df.columns]
    if missing:
        st.error(
            f"**This CSV is missing required column(s): {', '.join(missing)}.**\n\n"
            f"Columns found in the file instead: {', '.join(df.columns.tolist())}\n\n"
            "This usually means either a different dataset was uploaded/committed "
            "than the app expects, or the file didn't parse correctly (e.g. wrong "
            "delimiter, corrupted upload, or a Git LFS pointer file — see above). "
            "Upload a CSV with the expected columns via the sidebar to continue."
        )
        st.stop()

    # --- trim whitespace on text columns (fixes the 1,612 -> 17 sub-category issue) ---
    # pandas 3.0 introduced a dedicated "str" dtype separate from "object" —
    # select both explicitly so this works cleanly on both pandas 2.x and 3.x
    # without triggering a Pandas4Warning.
...
with tab_category:
    ...
    with col1:
        st.dataframe(cat.sort_values("Sales", ascending=False).style.format(
            {"Sales": "${:,.0f}", "Profit": "${:,.0f}", "Margin %": "{:.1f}%"}), width="stretch", hide_index=True)
    with col2:
